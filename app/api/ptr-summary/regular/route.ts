...
export async function GET(request: NextRequest) {
  ...
  if (session.role !== "regular") {
    return unauthorized("Regular user access required.", 403);
  }

  try {
    const filter = parseFilterFromRequest(request);
    const result = await getPtrSummaryForRegular(session.userId, filter);
    ...
    console.error("GET /api/ptr-summary/regular failed", error);
    return NextResponse.json(

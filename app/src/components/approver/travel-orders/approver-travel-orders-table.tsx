...
const ApproverTravelOrderDrawer = dynamic(
  () =>
    import("@/src/components/approver/travel-orders/approver-travel-order-drawer").then(
      (mod) => ({ default: mod.ApproverTravelOrderDrawer }),
    ),
  {
    ssr: false,
...
export function ApproverTravelOrdersTable({
  ...
  useEffect(() => {
    if (selectedOrder && selectedOrder.status.toUpperCase() === "CANCELLED") {
      showToast({
        ...
        description: `Travel order ${selectedOrder.orderNo} was cancelled by the requester.`,
      });
    }
  }, [selectedOrder, showToast]);
  const openDrawerFromTrigger = useCallback(
    (order: ApproverTravelOrderItem, triggerElement?: HTMLElement | null) => {
      if (triggerElement) {
  ...
  const shouldVirtualizeRows = paginatedRows.length > VIRTUAL_SCROLL_THRESHOLD;
  const {

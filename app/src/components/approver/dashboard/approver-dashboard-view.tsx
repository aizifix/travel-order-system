...
// Separate component for metrics - can be used with its own Suspense
export function ApproverDashboardMetrics({
  metrics,
}: {
...
// Separate component for table - can be used with its own Suspense
export function ApproverDashboardTable({
  recentOrders,
}: {
  ...
  return (
    <section aria-labelledby="recent-assigned-requests-heading">
      ...
      <div className="overflow-x-auto overflow-y-auto rounded-2xl border border-[#dfe1ed] bg-white max-h-[calc(100vh-300px)]">
...
export function ApproverDashboardView({
  ...
  return (
    <div className="space-y-8 lg:space-y-10">
      {/* Welcome section can be rendered by parent for instant loading */}
      {fullName && (
      ...
      <ApproverDashboardMetrics metrics={metrics} />
      <ApproverDashboardTable recentOrders={recentOrders} />
    </div>
  );
}
